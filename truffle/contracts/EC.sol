...
contract EC {
    ...
    uint256 constant public ORDER = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141;
    ...
    function _invF(uint256 val) public pure
        returns(uint256 invVal)
    {
        uint256 t = 0;
        uint256 newT = 1;
        ...
        while (newR != 0) {
            ...
            (t, newT) = (newT, addmod(t, (ORDER - mulmod(q, newT, ORDER)), ORDER));
            (r, newR) = (newR, r - q * newR );
        }
	...
	returns (uint256 negx, uint256 negy) {
	if (isZero(x,y)) {
	    (negx, negy) = setInfinity(); 
	} else {
	    negx = x;
	    negy = addmod(x, y, ORDER);
	}
    }
