...
import static com.ing.blockchain.zk.components.CFT.TWO;
import static com.ing.blockchain.zk.components.CFT.l;
import static com.ing.blockchain.zk.components.CFT.t;
...
public class ExportUtil {
    ...
    public static List<BigInteger> toRedundantList(BoudotRangeProof p, Commitment commitment, ClosedRange range) {
        ...
        int T = 2 * (t + l + 1) + (range.getEnd().subtract(range.getStart())).bitLength();
        ...
        ints.add(p.getSqrProofLeft().getECProof().getD1().signum() == -1 ? ONE : TWO);
        ints.add(p.getSqrProofLeft().getECProof().getD2().signum() == -1 ? ONE : TWO);
        ints.add(p.getSqrProofRight().getECProof().getD1().signum() == -1 ? ONE : TWO);
        ints.add(p.getSqrProofRight().getECProof().getD2().signum() == -1 ? ONE : TWO);
        ints.add(p.getCftProofLeft().getD2().signum() == -1 ? ONE : TWO);
        ints.add(p.getCftProofRight().getD2().signum() == -1 ? ONE : TWO);
